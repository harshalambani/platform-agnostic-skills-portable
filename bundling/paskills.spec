...
datas = [
    ...
    # Canonical (shipped) ITR rules base -- NOT DefaultData: this must be
    # replaced wholesale on every update, not frozen at first run (2026-07-24
    # handover: ship canonical ITR rules inside App\). Lands at
    # _MEIPASS/itr/{rules/*.yaml, *.example.yaml}; read via
    # rules.canonical_rules_dir() / ui/_config.canonical_itr_rules_dir().
    (str(PROJECT_ROOT / "bundling" / "canonical" / "itr"), "itr"),
