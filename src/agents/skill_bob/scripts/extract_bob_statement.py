...
def _require_pdfplumber() -> None:
    ...
    collection (via tests/skill_bob/), and a SystemExit raised at collection
    time is an INTERNALERROR that aborts the WHOLE session -- every other test
    in the run, silently. Deferring the check to main() keeps the CLI's
    behaviour identical while making the module safe to import anywhere.
    """
