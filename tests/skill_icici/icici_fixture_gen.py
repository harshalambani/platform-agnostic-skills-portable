...
# ---------------------------------------------------------------------------
# Excel-re-save fixture builder (IMP-03): writes Value Date/Transaction Date
# and/or the amount columns as REAL Excel DATE/NUMBER cells for selected
# rows, instead of the plain text cells the genuine net-banking download
# always uses -- simulating a user opening the statement in Excel and
# re-saving it, which silently converts those text cells to typed ones.
# ---------------------------------------------------------------------------
...
def _comma_date_to_pydate(s: str):
    """Parse a fixture's own "DD,Mon,YYYY" string into a real datetime.date,
    for writing as a typed Excel DATE cell."""
    import datetime as _dt
    day, mon, year = s.split(",")
    return _dt.date(int(year), _COMMA_MONTH_ABBR[mon], int(day))


def build_xls_with_cell_types(
    transactions: list[tuple[str, str, str, str, str, str, str]],
    period_from_raw: str,
    period_to_raw: str,
    *,
    date_rows: set[int] | None = None,
    number_rows: set[int] | None = None,
    account_number: str = SYN_ACCOUNT_NUMBER,
) -> bytes:
    """Like :func:`build_xls_for`, but for the 1-based row numbers in
    ``date_rows`` writes Value Date/Transaction Date as real Excel DATE
    cells (xlrd reads these back as ``XL_CELL_DATE``), and for row numbers
    in ``number_rows`` writes Withdrawal/Deposit/Balance as real NUMBER
    cells (``XL_CELL_NUMBER``) instead of text -- the shape a genuine
    all-text statement takes on after being opened and re-saved in Excel.
    Rows not listed keep the original text-cell form, so a single fixture
    can mix both shapes in one file.
    """
    import xlwt

    date_rows = date_rows or set()
    number_rows = number_rows or set()
    ...
    for i, (vdate, tdate, cheque, remarks, wdl, dep, bal) in enumerate(transactions, start=1):
        ...
        ws.write(r, 1, str(i))
        if i in date_rows:
            ws.write(r, 2, _comma_date_to_pydate(vdate), date_style)
            ws.write(r, 3, _comma_date_to_pydate(tdate), date_style)
        else:
            ws.write(r, 2, vdate)
            ws.write(r, 3, tdate)
        ws.write(r, 4, cheque)
        ws.write(r, 5, remarks)
        if i in number_rows:
            ws.write(r, 6, float(wdl) if wdl else 0.0)
            ws.write(r, 7, float(dep) if dep else 0.0)
            ws.write(r, 8, float(bal) if bal else 0.0)
        else:
            ws.write(r, 6, wdl)
            ws.write(r, 7, dep)
            ws.write(r, 8, bal)
...
def build_xlsx_with_cell_types(
    transactions: list[tuple[str, str, str, str, str, str, str]],
    period_from_raw: str,
    period_to_raw: str,
    *,
    ...
    """Build a .xlsx (openpyxl) with the same preamble/header/data-row
    layout as the real ICICI .xls, but as a genuine zip/XML container
    (magic bytes 'PK') -- simulating the user re-saving the statement as
    .xlsx in Excel. Dates and amounts are written as real typed cells
    (openpyxl's natural behaviour for python date/float values), matching
    what a real Excel re-save would produce.
    """
    import openpyxl
    ...
    def _w(row0, col0, value):
        # openpyxl is 1-indexed; row0/col0 here are the 0-indexed
        # coordinates used throughout this module, matching xlwt's ws.write.
        ws.cell(row=row0 + 1, column=col0 + 1, value=value)

    _w(0, 1, "DETAILED STATEMENT")
    _w(1, 1, "Search")
    _w(2, 1, "")
    _w(3, 1, "Account Number")
    _w(3, 3, f"{account_number}(INR)  - SYNTHETIC BRANCH")
    _w(4, 1, "Transaction Date from")
    _w(4, 3, period_from_raw)
    _w(4, 4, "to")
    _w(4, 5, period_to_raw)
    _w(5, 1, "Transaction Period")
    _w(6, 1, "Advanced Search")
    _w(7, 1, "Amount from")
    _w(8, 1, "Cheque number from")
    _w(9, 1, "Transaction remarks")
    _w(10, 1, "Transaction type")
    _w(11, 1, "Transactions List -")
    _w(11, 8, "")
    ...
    for i, (vdate, tdate, cheque, remarks, wdl, dep, bal) in enumerate(transactions, start=1):
        r = 12 + i
        _w(r, 1, str(i))
        _w(r, 2, _comma_date_to_pydate(vdate))
        _w(r, 3, _comma_date_to_pydate(tdate))
        _w(r, 4, cheque)
        _w(r, 5, remarks)
        _w(r, 6, float(wdl) if wdl else 0.0)
        _w(r, 7, float(dep) if dep else 0.0)
        _w(r, 8, float(bal) if bal else 0.0)
