...
- **`<accrual_journal_path> (optional -- H35-02, only written if accrual_journal_path is set AND an accrual was computed)`** — A SEPARATE one-transaction GnuCash multi-split journal CSV, dated 31 March of the financial year: the year-end share-of-profit accrual per the LLP statement of account. Same import settings as journal_path's file, above. Never contains the monthly transactions -- import both files if both were produced.
